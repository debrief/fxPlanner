...
- [x] T036 [US1] Create MapPanel in src/main/java/com/planetmayo/usvsim/view/MapPanel.java with **WebView + Leaflet.js** integration. Features: (1) Functional Leaflet map displaying Portland Harbour with OSM tiles, (2) Pan/zoom controls working (buttons), (3) Polygon rendering support, (4) Track visualization. Uses CDN-hosted Leaflet libraries. Offline tiles deferred for future optimization. **E2E test verifies MapView is present and functional** (not placeholder).
- [x] T037 [US1] Implement DrawingController in src/main/java/com/planetmayo/usvsim/controller/DrawingController.java with **Leaflet.Draw integration** for polygon drawing. Features: (1) Leaflet.Draw library provides UI for polygon creation, (2) JavaScript callback captures completed polygons, (3) Converts drawn shapes to Java Polygon model, (4) Supports polygon and waypoint drawing modes. **E2E test verifies drawing interaction works on actual map**.
