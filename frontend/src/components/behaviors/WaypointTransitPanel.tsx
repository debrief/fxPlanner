...
interface WaypointTransitPanelProps {
  onConfirm: (waypoints: Waypoint[]) => void;
  onCancel: () => void;
}

/**
 * Panel for configuring Waypoint Transit behavior.
 * Allows user to add waypoints via map interaction and set speeds.
 * Embedded in tabbed panel (no modal wrapper).
 */
const WaypointTransitPanel: React.FC<WaypointTransitPanelProps> = ({
  onConfirm,
  onCancel,
}) => {
  const [waypoints, setWaypoints] = useState<Waypoint[]>([]);
  const [defaultSpeed, setDefaultSpeed] = useState<number>(4.0); // 4 m/s default (~7.8 knots)
  const { drawnWaypoints, startWaypointDrawing, clearDrawing } = useMapContext();

  // Sync map waypoints to panel when they change
  useEffect(() => {
  ...
  return (
    <div className="behavior-panel">
      <div className="panel-header">
        <h3>Configure Waypoint Transit</h3>
      </div>
...
export default WaypointTransitPanel;
